...
def objective_factory(X_tr, y_tr, groups_tr):
    def objective(trial):
        ...
        # StandardScaler + PCA on outer-train subset
        ...
        # Inner CV (GroupKFold on outer-train)
...
for train_idx, val_idx in gkf_outer.split(X_train_enc, y_train, groups_train):
    ...
    # Refit scaler + PCA on outer-train
    scaler = StandardScaler()
    X_tr_scaled = scaler.fit_transform(X_tr)
    ...
    # Train model with best params on outer-train
    model = MLP(
    ...
    loader = DataLoader(
        TensorDataset(
            torch.tensor(X_tr_pca, dtype=torch.float32),
    ...
    with torch.no_grad():
        y_pred_val = model(
            torch.tensor(X_val_pca, dtype=torch.float32).to(device)
        ).cpu().numpy().flatten()
...
with torch.no_grad():
    y_pred_train = best_model_overall(
        torch.tensor(X_train_pca, dtype=torch.float32).to(device)
    ).cpu().numpy().flatten()
...
# Nested CV results
print(
    f"\nNested PyTorch MLP + PCA CV MAE: {np.mean(outer_mae_scores):.3f} "
    f"+/- {np.std(outer_mae_scores):.3f}"
)
print(
    f"Nested PyTorch MLP + PCA CV RMSE: {np.mean(outer_rmse_scores):.3f} "
