"""
KNN Regression with PCA + Optuna Tuning for hyperparameters

Method summary:
- Load tabular data combined with SBERT description embeddings (via data_s3_utils)
- Perform nested 5-fold GroupKFold CV:
    * Inner loop: Optuna optimization of KNN + PCA hyperparameters
    * Outer loop: unbiased MAE/RMSE estimation
- Retrain best model on full training set using optimal hyperparameters
- Report training performance and selected hyperparameters
"""
...
# Train final model on full dataset (with CV-tuned hyperparameters)
