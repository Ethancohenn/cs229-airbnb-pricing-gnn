...
def objective_factory(X_tr, y_tr, groups_tr):
    def objective(trial):
        ...
        # PyTorch loader
        X_tensor = torch.tensor(X_tr_pca, dtype=torch.float32)
        y_tensor = torch.tensor(y_tr.reshape(-1,1), dtype=torch.float32)
        loader = DataLoader(TensorDataset(X_tensor, y_tensor),
                            batch_size=batch_size, shuffle=True)

        # Model
        model = MLP(n_components, hidden1, hidden2, hidden3, dropout).to(device)
...
for train_idx, val_idx in gkf_outer.split(X_train_enc, y_train, groups_train):
    ...
    model = MLP(best_params["n_components"],
                best_params["hidden1"],
                best_params["hidden2"],
                best_params["hidden3"],
    ...
    loader = DataLoader(
        TensorDataset(torch.tensor(X_tr_pca, dtype=torch.float32),
    ...
    model.eval()
    with torch.no_grad():
        y_pred_val = model(torch.tensor(X_val_pca, dtype=torch.float32).to(device)).cpu().numpy().flatten()
    ...
    if val_mae < best_val_mae_overall:
        ...
        best_pca_overall = pca

# ===========================
# Final training error using best model overall
# ===========================
X_train_scaled = best_scaler_overall.transform(X_train_enc)
X_train_pca = best_pca_overall.transform(X_train_scaled)

best_model_overall.eval()
with torch.no_grad():
    y_pred_train = best_model_overall(torch.tensor(X_train_pca, dtype=torch.float32).to(device)).cpu().numpy().flatten()
...
# Nested CV results
print(f"\nNested PyTorch MLP + PCA CV MAE: {np.mean(outer_mae_scores):.3f} +/- {np.std(outer_mae_scores):.3f}")
print(f"Nested PyTorch MLP + PCA CV RMSE: {np.mean(outer_rmse_scores):.3f} +/- {np.std(outer_rmse_scores):.3f}")


#Final Training MAE (best fold model): 0.313
#Final Training RMSE (best fold model): 0.419
#Best hyperparameters (best fold model): {'n_components': 10, 'hidden1': 154, 'hidden2': 103, 'hidden3': 26, 'dropout': 0.08811290706706715, 'lr': 0.0022317426276472186, 'batch_size': 128}

#Nested PyTorch MLP + PCA CV MAE: 0.427 +/- 0.022
#Nested PyTorch MLP + PCA CV RMSE: 0.557 +/- 0.030