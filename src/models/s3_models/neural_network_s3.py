"""
PyTorch MLP Regression with Optuna Hyperparameter Tuning (Nested Geocluster CV)
(using tabular features + description embeddings)

Method summary:
- Load tabular data + SBERT description embeddings (via data_s3_utils)
- Define a 3-layer MLP with batch normalization and dropout
- Perform nested cross-validation with GroupKFold:
    * Inner loop (Optuna): tune hidden sizes, dropout, learning rate, batch size
    * Outer loop: obtain unbiased MAE/RMSE under geocluster splits
- Select the best outer-fold model and scaler
- Report training performance (MAE, RMSE) and nested CV metrics
"""
