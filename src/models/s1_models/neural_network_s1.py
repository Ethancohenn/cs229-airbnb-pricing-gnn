"""
PyTorch MLP Regression with Optuna Tuning (Random CV, S1)

Method summary:
- Load S1 tabular data, one-hot encode features, push tensors to CPU/CUDA
- Use nested KFold CV with Optuna to tune layer sizes, dropout, lr, batch size
...
# --------------------
# Optuna objective factory (inner CV)
# --------------------
def objective_factory(X_tr, y_tr):
    def objective(trial):
        ...
        # Inner 3-fold random CV
        kf_inner = KFold(n_splits=3, shuffle=True, random_state=229)
        mae_scores = []
        for _, idx_val in kf_inner.split(X_tr, y_tr):
...
# --------------------
# Outer 5-fold random CV
# --------------------
kf_outer = KFold(n_splits=5, shuffle=True, random_state=229)
...
for fold, (train_idx, val_idx) in enumerate(kf_outer.split(X_train_enc, y_train), start=1):
    X_tr, X_val = X_train_enc[train_idx], X_train_enc[val_idx]
    y_tr, y_val = y_train[train_idx], y_train[val_idx]

    study = optuna.create_study(direction="minimize")
    study.optimize(objective_factory(X_tr, y_tr), n_trials=10, show_progress_bar=False)
    ...
    print(f"Fold {fold} completed | CV MAE: {val_mae:.3f}, RMSE: {val_rmse:.3f} | Best params: {best_params}")
